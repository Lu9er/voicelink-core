...
const milestones = [
  {
    date: "In progress",
    title: "Common Voice submission and live call ingestion",
    description:
      "Processing retained speech clips for submission to the Mozilla Common Voice dataset. In parallel, building partner-facing analytics dashboards and testing live call-in data ingestion from radio stations to complement the archive-based pipeline.",
    active: true,
  },
  {
    date: "April 2026",
  ...
  },
  {
    ...
    title: "Voice of Teso data ingestion initiated",
    description:
      "Began initial data ingestion from Voice of Teso archives for the Ateso language. This track was paused to prioritise the CBS Luganda archive processing at scale.",
    active: false,
  },
  {
    date: "February 2026",
            ...
            {milestones.map((m, i) => (
              <div key={i} className="relative pl-10 pb-12 last:pb-0">
                ...
                <div className={`absolute left-0 top-1.5 h-6 w-6 rounded-full flex items-center justify-center ${
                  m.active ? "bg-tertiary/20" : "bg-secondary/10"
                }`}>
                  <div className={`h-2.5 w-2.5 rounded-full ${
                    m.active ? "bg-tertiary animate-pulse" : "bg-secondary"
                  }`} />
                </div>
                {/* Content */}
                <p className={`text-xs font-semibold uppercase tracking-wider mb-2 font-[family-name:var(--font-label)] ${
                  m.active ? "text-tertiary" : "text-secondary"
                }`}>
                  {m.date}
                </p>
